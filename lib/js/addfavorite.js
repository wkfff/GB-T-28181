    ...
    if (!window.addfavoriteWin)
    {
        ...
        addfavoritePanle.ligerForm({
            fields: [
                 ...
                 { display: "收藏别名", name: "FavoriteTitle", newline: true, labelWidth: 100, width: 220, space: 30, type: "text",
                     validate: { required: true, messages: { required: '别名不能为空'} }
                  }
    ...
    { 
        ...
        if (addfavoritePanle.valid() && manager)
        {
            LG.ajax({
                ...
                data: { MenuID: manager.getValue(), FavoriteTitle: $("#FavoriteTitle").val() },
                success: function ()
                {
                    LG.showSuccess('收藏成功');
                    window.addfavoriteWin.hide();

...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        enabledEdit: false,                      //是否允许编辑。为true时不可点击grid中内容而选中行
        isScroll: true,                         //是否滚动 
        ...
        //onXLoadData: null,                    //测试，增加自定义触发函数，以探求绑定和触发机制
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        addRowFast: function (rowdata, neardata, isBefore, parentRowData)
        {
            var g = this, p = this.options;
            rowdata = rowdata || {};
            g._addData(rowdata, parentRowData, neardata, isBefore);
        ...
        upgrade: function (rowParm)
        {
            ...
            $("> td > div > .l-grid-tree-space:last", rowobjs).addClass("l-grid-tree-link l-grid-tree-link-close");//--校兵
        },
        ...
        toggle: function (rowParm)
        {
            ...
            function toggleChildren(items)
            {
                for (var i = 0, l = items.length; i < l; i++)
                {
                    ...
                    if (haschildren)
                    {
                        //如果这个子节点原来是折叠状态的,那么子节点的子节点不做处理，即不对子节点进行递归展开。
                        if ($(".l-grid-tree-link", currentRow).hasClass("l-grid-tree-link-close"))
                        {
                            currentRow.show();
                        }
                        //如果子节点原来是展开状态的，则递归展开子节点树。
                        else
                        {
                            currentRow.show();
                    ...
                    {
                        //如果这个子节点不是叶子节点（进行过upgrade()操作，isLeaf()返回false，），但是还没有其子节点，则注释掉的代码
                        //的作用是将这个子节点展开（即使看不到其孩子）。
        ...
        {
            var g = this, p = this.options;
            if(g.currentData!=null)g.currentData[p.root] = [];
        },
        clearGrid: function()
        {
            var g = this, p = this.options;
